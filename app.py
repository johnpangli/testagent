...
with st.sidebar:
    ...
    st.caption("v2.0 // Bulletproof Edition")
    st.markdown("---")
    
    # --- SECURE API HANDLING ---
    # This looks for keys in Streamlit Secrets first
    with st.expander("🔑 API Keys", expanded=False):
        
        # Check if secrets exist, otherwise default to empty
    ...
    run_btn = st.button("🚀 Run Analysis Engine", type="primary")
...
if run_btn:
    if not GEMINI_API_KEY or not CENSUS_API_KEY:
        ...
        with st.status("⚙️ Agent Working...", expanded=True) as status:
            ...
            demographics_df = get_demographics(CENSUS_API_KEY, TARGET_REGION)
            
            st.write(f"🛒 Scouting Market Data for '{TARGET_CATEGORY}'...")
            market_df = get_market_data(TARGET_CATEGORY)
            
            st.write("📄 Ingesting Trend Reports...")
            trends_text = process_trends(uploaded_files)
            status.update(label="✅ Data Acquisition Complete", state="complete", expanded=False)

        if demographics_df is not None and not market_df.empty:
            st.divider()
            
            m1, m2, m3 = st.columns([1,1,2])
            m1.metric("Avg Income", f"${demographics_df['income'].mean():,.0f}")
            m2.metric("Poverty Rate", f"{demographics_df['poverty_rate'].mean():.1f}%")
            
            brand_list = sorted(market_df['brand_clean'].unique().tolist())
            my_brand = m3.selectbox("Select Your Brand Focus:", brand_list)

            comp_df = market_df[market_df['brand_clean'] != my_brand]
            if not comp_df.empty:
                top_movers = comp_df.groupby('brand_clean')['unique_scans_n'].sum().sort_values(ascending=False).head(5).index.tolist()
            else:
                top_movers = []
            ...
            if st.button("✨ Generate Strategic Directive", type="primary"):
                with st.spinner("🧠 Synthesizing Strategy..."):
                    ...
                    def get_summary(b_name):
                        d = market_df[market_df['brand_clean'] == b_name].head(5)
                        summary = []
                    ...
                    DATA: Income ${demographics_df['income'].mean():,.0f}. MY BRAND: {my_brand}. COMPETITORS: {top_movers}.
                    TRENDS: {trends_text}.
